...
- **Cloudflare WAF check**: Cloudflare's default rules can silently block AI crawlers even when `robots.txt` allows them — a common invisible GEO blocker
...
- **Query fan-out detection**: GSC regex to surface long-tail conversational queries generated by AI sub-query decomposition — with or without GSC MCP
- **Brand mentions vs backlinks** (Ahrefs Dec 2025, 75K brands): brand mentions correlate 3× more with AI citations than backlinks — YouTube mentions show strongest correlation across all AI platforms
